---
title: ServiceNow ナレッジ GraphコネクタMicrosoft Search
ms.author: kam1
...
# <a name="servicenow-knowledge-graph-connector"></a>ServiceNow ナレッジ Graph コネクタ
...
この記事は、Microsoft 365管理者または ServiceNow ナレッジ サーバー コネクタを構成、実行、および監視するGraphです。 この記事では、「コネクタ コネクタのセットアップ」に記載されている一般的Graph[補足](configure-connector.md)します。 まだ実行していない場合は、「コネクタのセットアップ」の記事Graph一般的なセットアップ プロセスを理解してください。
...
ServiceNow ナレッジ Graphコネクタには、最新のリリースで次の制限があります。
