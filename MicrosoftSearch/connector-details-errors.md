...
1004 | オンプレミスのデータ ゲートウェイに到達できません。 ゲートウェイ サービスが実行され、ゲートウェイの詳細が接続構成で更新されます。 | ゲートウェイを使用してコンピューターを確認し、Power BI Gateway アプリケーションを開き、ゲートウェイが実行されている状態を確認します。 ゲートウェイが Microsoft Search と同じ管理者アカウントを使用している場合は、すべてのゲートウェイの詳細が接続構成で更新されている必要があります。
...
1007 | 有効な Power BI ライセンスが検出されません。 このクロールを実行するには、有効な Power BI ライセンスが必要です。 | このクロールを実行するには、有効な Power BI ライセンスが必要です。 組織に有効なライセンスが含されていないことを確認します。 その場合は、もう一度やり直してください。 ライセンスが取得できない場合は、ライセンスを取得してから、もう一度やり直してください。
...
1011 | Graph コネクタ エージェント [が](on-prem-agent.md) 到達可能またはオフラインではありません。 | 
...
5000 | 何か問題が起きました。 このままの場合は、サポートにお問い合わせください。 |