...
検索垂直は、複数のコネクタソースからの結果を表示できるようになります。 これにより、検索結果ページをより柔軟に設計できるようになります。 縦書きセットアップの既存の管理環境では、"コンテンツソース" の手順で複数の接続を選択することができます。
可能な限り多くの意味ラベルを正確に配置すると、この状況が改善されます。 スキーマ定義と取り込み時に、セマンティックラベルを追加することができます。

[ここ](#configure-connector-step-5-assign-property-labels) では、セマンティックラベルを作成して管理する方法についての追加情報を示します。

### <a name="things-you-should-know"></a>知っておくべきこと

1. 接続は、1つの垂直方向にのみコンテンツソースとして追加できます。 複数の業種で接続を再利用することは許可されていません。
2. 複数の接続ソースが追加されている垂直方向の検索のクエリを設定する必要がある場合は、一般的なソースプロパティを使用してそのようなクエリを作成する必要があります。
...
4. ナビゲーションウィンドウで、[ **ユーザー設定の操作**] に移動し、[ **垂直** ] タブを選択します。
5. 垂直方向を追加するには、[ **追加**] を選択します。
...
### <a name="default-search-result-layout"></a>既定の検索結果のレイアウト

コネクタを構成しているときに、 **ラベル** と **コンテンツ** のプロパティがソースプロパティに正しくマップされている場合、コネクタコンテンツに既定の検索結果レイアウトが表示されます。 ラベル **タイトル** は、最も重要なラベルです。 既定の検索結果のレイアウトを使用するには、このラベルにプロパティを割り当てることを **強くお勧め** します。

### <a name="create-your-own-result-type"></a>独自の検索結果の種類を作成する

検索結果のレイアウトを独自に作成し、 **結果の種類** を作成することで、既定の検索結果のレイアウトを上書きすることができます。 検索結果の種類とは、検索結果のタイプの違いに応じて表示方法を変えるルールのことです。 このルールは、次のような構成になっています。

- 検索結果のコンテンツソースなど、各検索結果を比較するための **1 つ以上の条件**。  
- 条件を満たす検索結果に使用する **結果レイアウト**。 結果レイアウトは、条件を満たすすべての結果が検索結果ページに表示され、動作する方法を制御します。

**既定の検索結果レイアウトを表示するために適切なマッピングが行われていない場合は、垂直方向に表示する結果に対して、少なくとも1つの結果の種類を作成する必要があります。** 業種ごとに複数の結果タイプを作成できます。これにより、さまざまなタイプの結果にさまざまなレイアウトを使用できます。 たとえば、*重大度 1* のインシデントをカスタマイズして、*重大度 3* のインシデントと比較してより目立つ色と大きなフォントを使用できます。
...
2. **検索結果の種類** を追加するには、[**追加**] を選択します。 結果タイプを編集するには、関連するリストで結果タイプを選択します。
...
4. ナビゲーションウィンドウで、[ **ユーザー設定の操作**] に移動し、[ **結果の種類** ] タブを選択します。
5. 検索結果の種類を追加するには、[ **追加**] を選択します。  または、結果タイプを編集するには、リストから結果タイプを選択します。

## <a name="step-3-view-the-vertical-after-its-enabled"></a>手順 3: 有効にした縦の表示を表示する
